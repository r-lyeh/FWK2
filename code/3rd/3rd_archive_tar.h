...
tar_t *tar_open(const char *filename, const char *mode);

    int tar_find(tar_t*, const char *entryname); // returns entry number; or <0 if not found.
    unsigned tar_count(tar_t*);
        char*    tar_name(tar_t*, unsigned index);
        unsigned tar_size(tar_t*, unsigned index);
        unsigned tar_offset(tar_t*, unsigned index);
        void*    tar_extract(tar_t*, unsigned index); // must free() after use

void tar_close(tar_t *t);
...
tar_t *tar_open(const char *filename, const char *mode) {
    ...
    tar_t zero = {0}, *t = REALLOC(0, sizeof(tar_t));
...
int tar_find(tar_t *t, const char *entryname) {
...
unsigned tar_count(tar_t *t) {
    return t ? t->count : 0;
}

char* tar_name(tar_t *t, unsigned index) {
    return t && index < t->count ? t->entries[index].filename : 0;
}

unsigned tar_size(tar_t *t, unsigned index) {
    return t && index < t->count ? t->entries[index].size : 0;
}

unsigned tar_offset(tar_t *t, unsigned index) {
    return t && index < t->count ? (unsigned)t->entries[index].offset : 0;
}

void *tar_extract(tar_t *t, unsigned index) {
    if( t && index < t->count ) {
