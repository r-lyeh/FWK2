...
pak_t* pak_open(const char *fname, const char *mode /*a,r,w*/);

    // (w)rite or (a)ppend modes only
    int pak_append_file(pak_t*, const char *filename, FILE *in);
    int pak_append_data(pak_t*, const char *filename, const void *in, unsigned inlen);

    // (r)ead only mode
    int pak_find(pak_t*,const char *fname); // return <0 if error; index otherwise.
    unsigned pak_count(pak_t*);
        unsigned pak_size(pak_t*,unsigned index);
        unsigned pak_offset(pak_t*, unsigned index);
        char *pak_name(pak_t*,unsigned index);
        void *pak_extract(pak_t*, unsigned index); // must free() after use

void pak_close(pak_t*);
...
pak_t *pak_open(const char *fname, const char *mode) {
    struct stat buffer;
    ...
    pak_t *p = malloc(sizeof(pak_t)), zero = {0};
...
int pak_append_data(pak_t *p, const char *filename, const void *in, unsigned inlen) {
...
int pak_append_file(pak_t *p, const char *filename, FILE *in) {
    // index meta
    unsigned index = p->count++;
    p->entries = REALLOC(p->entries, p->count * sizeof(pak_file));
    pak_file *e = &p->entries[index], zero = {0};
...
int pak_find(pak_t *p, const char *filename) {
    if( p->in ) {
...
unsigned pak_count(pak_t *p) {
    return p->in ? p->count : 0;
}

unsigned pak_offset(pak_t *p, unsigned index) {
    return p->in && index < p->count ? p->entries[index].offset : 0;
}

unsigned pak_size(pak_t *p, unsigned index) {
    return p->in && index < p->count ? p->entries[index].size : 0;
}

char *pak_name(pak_t *p, unsigned index) {
    return p->in && index < p->count ? p->entries[index].name : NULL;
}

void *pak_extract(pak_t *p, unsigned index) {
