...
#define DEFAULT_VSYNC WINDOW_VSYNC
#endif
...
void window_drop_callback(GLFWwindow* window, int count, const char** paths) {
    ...
    mkdir( pathdir, 0777 );
#else
    snprintf(pathdir, DIR_MAX, ".import/%llu_%s/", (unsigned long long)date(), ifdef(linux, getlogin(), getenv("USERNAME")));
    mkdir( ".import/", 0777 );
    mkdir( pathdir, 0777 );
#endif
...
int window_swap() {
    ...
    if( 
        ...
        cook_done && 
#endif
        ++capture_frame == tests_captureframes() ) {
