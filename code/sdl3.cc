...
static ImVec4 clear_color = ImVec4(32/255., 32/255., 32/255., 1.);
/*static*/ int keep = 1;
...
int sdl3win_init(float scale, unsigned flags) {
    ...
    // SDL_INIT_EVERYTHING
    if (!SDL_Init(SDL_INIT_VIDEO|SDL_INIT_AUDIO|SDL_INIT_GAMEPAD|SDL_INIT_JOYSTICK|SDL_INIT_EVENTS|SDL_INIT_CAMERA))
        die(SDL_GetError());
...
if(flags & WINDOW_TRANSPARENT) window_flags |= SDL_WINDOW_TRANSPARENT, clear_color = ImVec4(0,0,0,0);
...
ImFont* font = io.Fonts->AddFontFromFileTTF("/usr/share/fonts/liberation/LiberationSans-Regular.ttf", 16.0f);
#endif
#endif
