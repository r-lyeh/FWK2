...
// @todo: bring back im3d software rasterizer? (via dearimgui backend, brings AA lines despite whether MSAAx is used or not)
...
void ddraw_flush_projview(mat44 proj, mat44 view) {
    ...
    if(array_count(dd_text2d)) { // text 2d
        ...
        ortho44(mvp, -app_width()/2, app_width()/2, -app_height()/2, app_height()/2, -1, 1);
        translate44(mvp, -app_width()/2, app_height()/2, 0);
        glUniformMatrix4fv(glGetUniformLocation(dd_program, "u_MVP"), 1, GL_FALSE, mvp);
        for( int i = 0; i < 3; ++i ) { // [0] thin, [1] thick, [2] points
            GLenum mode = i < 2 ? GL_LINES : GL_POINTS;
...
void ddraw_camera(camera_t *cam) {
    ...
    perspective44(proj, cam->fov, app_width() / ((float)app_height()+!app_height()), 0.01f, 100.f);
