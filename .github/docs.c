...
#define strbegi(...) (!!strbegi(__VA_ARGS__))
#define file_read(x) file_read(x,0)
#define file_exist(x) is_file(x)
...
int main(int argc, char **argv) {
    ...
    if( DO_README ) {
        char *md = file_read(".github/README.md");
        strrepl(&md, "\r\n", "\n");
    ...
    preprocess_c(raw);
    // if(DO_DEBUG) fprintf(stdlog, "%s\n", raw);

    // split into lines
    array(char*) split = strsplit(raw, "\r\n");
    if(DO_DEBUG) fprintf(stdlog, "split %d lines\n", (int)array_count(split));
