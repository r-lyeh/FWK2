...
static array(ENetEvent) local_events;
...
void server_drop_client_peer(ENetPeer *peer) {
    struct peer_node_t *node = CAST(struct peer_node_t *, CALLOC(sizeof(struct peer_node_t), 1));
...
static
void server_push_loopback_event(ENetPacket *packet) {
    ENetEvent event = {0};

    // ensure we don't loopback MSG_BUF packets
    uint32_t mid = *(uint32_t*)(packet->data);

    if (mid == MSG_BUF)
        return;

    ENetPacket *packetCopy = enet_packet_create(packet->data, packet->dataLength, 0);
    event.packet = packetCopy;
    event.type = ENET_EVENT_TYPE_RECEIVE;
    array_push(local_events, event);
}
...
void server_broadcast_bin_flags(const void *msg, int len, uint64_t flags) {
    ...
    if (network_get(NETWORK_RANK) == 0) server_push_loopback_event(packet);
}
...
void server_send_bin(int64_t handle, const void *ptr, int len) {
    ...
    if (handle == 0 && network_get(NETWORK_RANK) == 0) {
        server_push_loopback_event(packet);
    }
    else {
...
void network_create(unsigned max_clients, const char *ip, const char *port_, unsigned flags) {
    ...
    if( !(flags&NETWORK_CONNECT) || flags&NETWORK_BIND ) {
        ...
        if( server_bind(max_clients, port) ) {
            ...
            if( socket >= 0 ) {
                ...
                if (!(flags&NETWORK_NOFAIL))
                    die("cannot neither connect to %s:%d, nor create a server", ip, port);
            }
        }
    } else {
        // client only
        ...
        if( socket > 0 ) {
            ...
            network_put(NETWORK_LIVE, 0);
            if (!(flags&NETWORK_NOFAIL))
                die("cannot connect to server %s:%d", ip, port);
        }
    }
...
static inline
char *server_process_event(ENetEvent event) {
    char *msg = 0;
    char ip[128] = {0};
    ...
    switch (event.type) {
        ...
        case ENET_EVENT_TYPE_RECEIVE: {
            char *dbg = event.peer ? (char *)event.peer->data : NULL;
            char *ptr = (char *)event.packet->data;
...
char** server_poll(unsigned timeout_ms) {
    ...
    for( ENetEvent event; Server && enet_host_service(Server, &event, timeout_ms) > 0; ) {
        ...
        msg = server_process_event(event);

        if(msg) array_push(events, va("%d %s", enet_event_to_netsync(event.type), msg));
    }

    // process loopback events
    for (int i = 0, size = array_count(local_events); i < size; i++) {
        char *msg = 0;

        msg = server_process_event(local_events[i]);

        if (msg) array_push(events, va("%d %s", enet_event_to_netsync(local_events[i].type), msg));
    }

    array_push(events, NULL);
    array_clear(local_events);
    return events;
}
