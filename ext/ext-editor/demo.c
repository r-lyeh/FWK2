...
// - game structure begins with root folder, and all nodes belong to them.
...
AUTORUN {
    ...
    obj_flag(objroot,|=,CAN_TICK|IS_TICKING|CAN_DRAW|IS_DRAWING|CAN_VIEW);
}
...
static
void _editor_attach(obj *o) { // @fixme: use objflags for these masks?
    ...
    obj_flag(o, |=, IS_OPEN);
    if( obj_hasmethod(o, tick) ) obj_flag(o, |=, CAN_TICK|IS_TICKING);
    if( obj_hasmethod(o, draw) ) obj_flag(o, |=, CAN_DRAW|IS_DRAWING);
    if( obj_hasmethod(o, view) ) obj_flag(o, |=, CAN_VIEW);

    for each_objchild(o, obj*, child) {
        _editor_attach(child);
    }
}
...
void list_all(list_all_t list, vec2 from, vec2 to) {
    ...
    fbo_bind(pick.fb.id);
        camera_push();
        editor_event_down(objroot, CAN_DRAW|IS_DRAWING, false, obj_draw, "i", RENDER_PASS_PICK);
        sprite_flush();
        camera_pop();
    fbo_unbind();
...
int editor( void (*game)(unsigned frame, float dt, double t) ) {
    ...
    editor_event_down(objroot, CAN_TICK|IS_TICKING, false, obj_tick, "d", dt);
    ...
    fx_begin();
    {
        ...
        editor_event_down(objroot, CAN_DRAW|IS_DRAWING, false, obj_draw, "i", default_render_pass);
        sprite_flush();
        camera_pop();
    }
    fx_end(0,0);
            ...
            camera_push();
                ...
                for each_array(selection,obj*,o)
                    while( (o = obj_parent(o)) != NULL )
                        if( obj_flag(o,&,CAN_VIEW) )
                            obj_flag(o,|=,IS_SELECTED);

                int default_render_pass = -1;
                editor_event_down(objroot, CAN_DRAW|IS_SELECTED, true, obj_draw, "i", default_render_pass);
