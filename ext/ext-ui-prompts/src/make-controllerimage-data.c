...
#include <errno.h>
    ...
    {
        DirHandle *dirp = (DirHandle *)calloc(1, sizeof (DirHandle));

        char buf[MAX_PATH];
        snprintf(buf, sizeof(buf), "%s/*", path);

        dirp->handle = FindFirstFileA(buf, &dirp->data);
    ...
    static const char *ReadDir(DirHandle *dirp)
    {
        if (dirp) {
            if (!dirp->count++) {
                // we already read the first entry, return that.
                return dirp->data.cFileName;
            } else if (FindNextFileA(dirp->handle, &dirp->data) != 0) {
                return dirp->data.cFileName;
            }
        }
        return NULL;
...
static char **break_string_into_lines(char *str)
{
    ...
    do {
        ...
        if (eos) {
            retval = (char **) realloc(retval, sizeof (char *) * (num_lines + 1));
...
static void parse_device_guids_file(DeviceInfo *device, const char *path)
{
    ...
    for (int i = 0; lines[i] != NULL; i++) {
        ...
        device->guids = (Guid *) realloc(device->guids, (device->num_guids + 1) * sizeof (Guid));
...
static void process_gamepad_dir(const char *devid, const char *path)
{
    ...
    if (!dirp) {
        ...
        fprintf(stderr, "%d Couldn't opendir '%s': %s\n", __LINE__, path, strerror(errno));
        exit(1);
    }
...
static void process_devicetype_dir(const char *devicetype, const char *path)
{
    ...
    char *fulltypepath = (char *) malloc(slen);
    ...
    if (!dirp) {
        ...
        fprintf(stderr, "%d Couldn't opendir '%s': %s\n", __LINE__, fulltypepath, strerror(errno));
        exit(1);
    }
...
int main(int argc, const char **argv)
{
    ...
    if (!dirp) {
        fprintf(stderr, "%d Couldn't opendir '%s': %s\n", __LINE__, basedir, strerror(errno));
        return 1;
    }
    ...
    while ((theme = ReadDir(dirp)) != NULL) {
        ...
        snprintf(fullpath, sizeof(fullpath), "%s/%s", basedir, theme);
