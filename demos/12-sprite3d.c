...
void game(unsigned frame) {
    ...
    if( !frame ) {
        ...
        cam2d = camera();
        cam2d.position = vec3(app_width()/2,app_height()/2,8);
        camera_enable(&cam2d);
        ...
        s1 = sprite_new("Captain Clown Nose.ini", (int[6]){KEY_UP,KEY_DOWN,KEY_LEFT,KEY_RIGHT,KEY_A,KEY_S});
        s2 = sprite_new("Crew-Crabby.ini", (int[6]){KEY_I,KEY_K,KEY_J,KEY_L} );
        s3 = sprite_new("Props-Shooter Traps.ini", (int[6]){0} );
        s4 = sprite_new("Crew-Fierce Tooth.ini", (int[6]){0,0,KEY_N,KEY_M} );
        ...
        // 2d pos and z-order
        // s1->pos = vec3(app_width()/2, app_height()/2, 2);
        // s2->pos = vec3(app_width()/2, app_height()/2, 1);
        // s3->pos = vec3(app_width()/2, app_height()/2, 1);
        // s4->pos = vec3(app_width()/2, app_height()/2, 1);
...
int main() {
    ...
    for( app_create(0.75, APP_MSAA8); app_swap(); ) {
        //if( input_down(KEY_Z) && input(KEY_ALT) ) app_send("app.record", __FILE__ ".mp4");
